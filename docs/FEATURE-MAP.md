...
3. `app/<item>/manifest.yml` — the manifest under test; its `installCmdSteps` execute inside the container
...
2. `app/<item>/manifest.yml` — the new manifest, following the pattern of existing items (e.g. `app/wordpress/manifest.json`)
